...
def has_cycle_constant(link):
    ...
    step = link
    double_step = link
    while step != Link.empty and double_step.rest!=Link.empty:
        step = step.rest
        double_step = double_step.rest.rest
        if double_step == Link.empty:
            break
        if step == double_step:
            return True
    return False
...
def reverse_other(t):
    ...
    def reverse_helper(t, need_reverse):
        if t.is_leaf():
            return
        reverse_label = [child.label for child in t.branches][::-1]
        for i in range(len(t.branches)):
            child = t.branches[i]
            reverse_helper(child, not need_reverse)
            if need_reverse:
                child.label = reverse_label[i]
