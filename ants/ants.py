...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        dst = 0
        def find_nearest(place, min_range, max_range, dst):
            if place.name == "Hive":
                return place
            if place.bees and min_range<=dst<=max_range:
                return place
            else:
                return find_nearest(place.entrance, self.min_range, self.max_range, dst+1)
        nearest = find_nearest(self.place, self.min_range, self.max_range, dst)
        if nearest == beehive:
            return None
        return rANTdom_else_none(nearest.bees) # REPLACE THIS LINE
        # END Problem 3 and 4
