class VendingMachine:
    ...
    def vend(self):
        if self.inventory:
            if self.funds > self.sell_price:
                charge = self.funds - self.sell_price
                self.funds = 0
                self.inventory -= 1
                return 'Here is your {0} and ${1} change.'.format(self.product, charge)
            elif self.funds == self.sell_price:
                self.funds = 0
                self.inventory -= 1
                return 'Here is your {0}.'.format(self.product)
            else:
                return 'You must add ${0} more funds.'.format(self.sell_price-self.funds)
...
def preorder(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    def print_helper(t, order):
        if t.is_leaf():
            order.append(t.label)
            return
        order.append(t.label)
        for child in t.branches:
            print_helper(child, order)
        return order
...
def path_yielder(t, value):
    ...
    for child in t.branches:
        for child_path in path_yielder(child, value):

            "*** YOUR CODE HERE ***"
            yield [t.label] + child_path
    if t.label ==value:
        yield [t.label]
