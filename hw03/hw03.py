...
def balanced(m):
    ...
    return balanced(end(left_arm)) and balanced(end(right_arm)) and length(left_arm)*left_weight == length(right_arm)*right_weight
...
def totals_tree(m):
    ...
    return tree(total_weight(m), [totals_tree(end(left(m)), end(right(m)))])
...
def has_path(t, word):
    ...
    br = True
    for branch in branches(t):
        br  = br or has_path(bracnch, word[1:])
    return br
